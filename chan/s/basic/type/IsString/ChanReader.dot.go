...
type ReaderROnlyChan interface { // receive-only channel
	RequestReader() (dat *strings.Reader)        // the receive function - aka "some-new-Reader-var := <-MyKind"
	TryReader() (dat *strings.Reader, open bool) // the multi-valued comma-ok receive function - aka "some-new-Reader-var, ok := <-MyKind"
}
