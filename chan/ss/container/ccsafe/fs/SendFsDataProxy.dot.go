...
// Note: SendProxyFsData imports "container/ring" for the expanding buffer.
import (
	"container/ring"
...
// SendProxyFsData returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyFsData(out chan<- *fs.FsData) chan<- *fs.FsData {
	proxy := make(chan *fs.FsData, FsDataCAP)
	go func() {
		n := FsDataQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- *fs.FsData
		var e *fs.FsData
		for {
			c = out
			if first == last {
