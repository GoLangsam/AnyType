...
// Note: SendProxyFsFoldS imports "container/ring" for the expanding buffer.
import (
	"container/ring"
...
// SendProxyFsFoldS returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyFsFoldS(out chan<- fs.FsFoldS) chan<- fs.FsFoldS {
	proxy := make(chan fs.FsFoldS, FsFoldSCAP)
	go func() {
		n := FsFoldSQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- fs.FsFoldS
		var e fs.FsFoldS
		for {
			c = out
			if first == last {
