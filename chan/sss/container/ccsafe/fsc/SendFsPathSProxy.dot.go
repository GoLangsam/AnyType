...
// Note: SendProxyFsPathS imports "container/ring" for the expanding buffer.
import (
	"container/ring"
...
// SendProxyFsPathS returns a channel to serve as a sending proxy to 'out'.
// Uses a goroutine to receive values from 'out' and store them
// in an expanding buffer, so that sending to 'out' never blocks.
//
// Note: the expanding buffer is implemented via "container/ring"
func SendProxyFsPathS(out chan<- fs.FsPathS) chan<- fs.FsPathS {
	proxy := make(chan fs.FsPathS, FsPathSCAP)
	go func() {
		n := FsPathSQUE // the allocated size of the circular queue
		first := ring.New(n)
		last := first
		var c chan<- fs.FsPathS
		var e fs.FsPathS
		for {
			c = out
			if first == last {
