...
func ChanFsFile(inp ...*fs.FsFile) (out <-chan *fs.FsFile) {
	...
	go func(out chan<- *fs.FsFile, inp ...*fs.FsFile) {
		defer close(out)
		for i := range inp {
			out <- inp[i]
		}
...
func ChanFsFileFuncNil(act func() *fs.FsFile) (out <-chan *fs.FsFile) {
	cha := make(chan *fs.FsFile)
	go func(out chan<- *fs.FsFile, act func() *fs.FsFile) {
		defer close(out)
		for {
...
func ChanFsFileFuncNok(act func() (*fs.FsFile, bool)) (out <-chan *fs.FsFile) {
	cha := make(chan *fs.FsFile)
	go func(out chan<- *fs.FsFile, act func() (*fs.FsFile, bool)) {
		defer close(out)
		for {
...
func ChanFsFileFuncErr(act func() (*fs.FsFile, error)) (out <-chan *fs.FsFile) {
	cha := make(chan *fs.FsFile)
	go func(out chan<- *fs.FsFile, act func() (*fs.FsFile, error)) {
		defer close(out)
		for {
...
func JoinFsFile(out chan<- *fs.FsFile, inp ...*fs.FsFile) (done <-chan struct{}) {
	...
	go func(done chan<- struct{}, out chan<- *fs.FsFile, inp ...*fs.FsFile) {
		defer close(done)
		for i := range inp {
			out <- inp[i]
		}
...
func JoinFsFileChan(out chan<- *fs.FsFile, inp <-chan *fs.FsFile) (done <-chan struct{}) {
	cha := make(chan struct{})
	go func(done chan<- struct{}, out chan<- *fs.FsFile, inp <-chan *fs.FsFile) {
		defer close(done)
		for i := range inp {
			out <- i
		}
...
func DoneFsFile(inp <-chan *fs.FsFile) (done <-chan struct{}) {
	cha := make(chan struct{})
	go func(done chan<- struct{}, inp <-chan *fs.FsFile) {
		defer close(done)
		for i := range inp {
			_ = i // Drain inp
		}
...
func DoneFsFileSlice(inp <-chan *fs.FsFile) (done <-chan []*fs.FsFile) {
	cha := make(chan []*fs.FsFile)
	go func(inp <-chan *fs.FsFile, done chan<- []*fs.FsFile) {
		defer close(done)
		FsFileS := []*fs.FsFile{}
		for i := range inp {
...
func DoneFsFileFunc(inp <-chan *fs.FsFile, act func(a *fs.FsFile)) (out <-chan struct{}) {
	...
	go func(done chan<- struct{}, inp <-chan *fs.FsFile, act func(a *fs.FsFile)) {
		defer close(done)
		for i := range inp {
			act(i) // Apply action
		}
...
func PipeFsFileBuffer(inp <-chan *fs.FsFile, cap int) (out <-chan *fs.FsFile) {
	...
	go func(out chan<- *fs.FsFile, inp <-chan *fs.FsFile) {
		defer close(out)
		for i := range inp {
			out <- i
		}
...
func PipeFsFileFunc(inp <-chan *fs.FsFile, act func(a *fs.FsFile) *fs.FsFile) (out <-chan *fs.FsFile) {
	...
	go func(out chan<- *fs.FsFile, inp <-chan *fs.FsFile, act func(a *fs.FsFile) *fs.FsFile) {
		defer close(out)
		for i := range inp {
			out <- act(i)
		}
...
func PipeFsFileFork(inp <-chan *fs.FsFile) (out1, out2 <-chan *fs.FsFile) {
	...
	go func(out1, out2 chan<- *fs.FsFile, inp <-chan *fs.FsFile) {
		defer close(out1)
		defer close(out2)
		for i := range inp {
			out1 <- i
