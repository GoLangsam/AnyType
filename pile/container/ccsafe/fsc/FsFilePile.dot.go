...
// NewS returns a (pointer to a) fresh FsFilePile
// of items (of type `*fs.FsFile`)
// with size as initial capacity
// and
// with buff non-blocking Add's before respective Next's
func FsFileNew(size, buff int) *FsFilePile {
	pile := new(FsFilePile)
...
// iff the FsFilePile is not Closed().
...
// Wait returns a done channel which emits the size (=length) of the FsFilePile once it's been closed.
// Users of Wait() must not iterate (via Next() and Reset()) before the returned done-channel is closed!
//
// Wait is a convenience - useful iff You do not like/need to start any traversal before the FsFilePile is fully populated.
// (Or iff You just like to know the size to be traversed, i.e. in order to allocate some traversal-related structure.)
//
// Note: Upon close of the done-channel, the FsFilePile is Reset() so You may start traversing it (via Next) right away.
