...
//
// Yoe may either
// traverse the DotPile lazily -following its (buffered) growth that is-
// or
...
// NewS returns a (pointer to a) fresh DotPile
// of items (of type `dot.Dot`)
// with size as initial capacity
// and
// with buff non-blocking Add's before respective Next's
func DotNew(size, buff int) *DotPile {
	pile := new(DotPile)
...
func (d *DotPile) Close() {
	close(d.pile)
}

// Wait returns a done channel which emits the size (=length) of the DotPile once it's been closed.
// Users of Wait() must not iterate (via Next() and Reset()) before the returned done-channel is closed!
//
// Wait is a convenience - useful iff You do not like/need to start any traversal before the DotPile is fully populated.
// (Or iff You just like to know the size to be traversed, i.e. in order to allocate some traversal-related structure.)
//
// Note: Upon close of the done-channel, the DotPile is Reset() so You may start traversing it (via Next) right away.
func (d *DotPile) Wait() (done <-chan int) {
	cha := make(chan int)
	go func(cha chan<- int, d *DotPile) {
		defer close(cha)
		d.Reset()
		defer d.Reset()
		for {
