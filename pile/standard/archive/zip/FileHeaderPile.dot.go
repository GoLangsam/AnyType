...
//
// Yoe may either
// traverse the FileHeaderPile lazily -following its (buffered) growth that is-
// or
...
// NewS returns a (pointer to a) fresh FileHeaderPile
// of items (of type `zip.FileHeader`)
// with size as initial capacity
// and
// with buff non-blocking Add's before respective Next's
func FileHeaderNew(size, buff int) *FileHeaderPile {
...
// iff the FileHeaderPile is not Closed().
...
func (d *FileHeaderPile) Close() {
	close(d.pile)
}

// Wait returns a done channel which emits the size (=length) of the FileHeaderPile once it's been closed.
// Users of Wait() must not iterate (via Next() and Reset()) before the returned done-channel is closed!
//
// Wait is a convenience - useful iff You do not like/need to start any traversal before the FileHeaderPile is fully populated.
// (Or iff You just like to know the size to be traversed, i.e. in order to allocate some traversal-related structure.)
//
// Note: Upon close of the done-channel, the FileHeaderPile is Reset() so You may start traversing it (via Next) right away.
func (d *FileHeaderPile) Wait() (done <-chan int) {
	cha := make(chan int)
	go func(cha chan<- int, d *FileHeaderPile) {
		defer close(cha)
		d.Reset()
		defer d.Reset()
		for {
