...
// PatSPile is a structure for
// a lazily populated sequence (= slice)
// of items (of type `[]struct{}`)
// which are cached in a growing-only list.
// Next() traverses the PatSPile.
// Reset() allows a new transversal from the beginning.
//
// You may either
// traverse the PatSPile lazily -following its (buffered) growth that is-
// or
...
func (d *PatSPile) Wait() (done <-chan int) {
	cha := make(chan int)
	go func(cha chan<- int, d *PatSPile) {
		defer close(cha)
		d.Reset()
